...
The `fg` and `bg` commands will no longer be able to reach it, though - you will need to refer to it by its process ID (PID) from now on.
...
1) Exit the terminal
2) log back in
3) then use the `ps` command to list the running processes

Verify the sleep command that is still running and its PID. Stop it with `kill` and the PID. In our example, the command is `kill 140`.
