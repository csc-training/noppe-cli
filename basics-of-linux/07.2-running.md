...
As discussed in [Processes](07-processes.md) chapter, running programs create a hierarchical structure that also applies to background processes.

A background process has the terminal session as its parent process. Exiting the terminal will also terminate all launched processes.

However, it may be desirable to keep some processes running when exiting the terminal (for example, when ending an SSH session). This can be achieved using programs such as `tmux` or `screen`, or commands such as `disown` and `nohup`.

#### disown

The `disown` command removes a job from the shell's job table, meaning the shell no longer owns it and it will continue to run even after the terminal has been exited.

Try out the following yourself:
...
{{USERNAME}}@{{HOSTNAME}}:~$ disown
{{USERNAME}}@{{HOSTNAME}}:~$ jobs
{{USERNAME}}@{{HOSTNAME}}:~$ █
:::

The job will have disappeared from the jobs list.

::: guidance
The `fg` and `bg` commands will no longer be able to reach it - you will need to refer to it by its process ID (PID) from now on.
:::
...
1) Exit the terminal
2) log back in
3) use the `ps` command to list the running processes
4) verify the sleep command that is still running and its PID.

Stop the process with `kill` and the PID. In our example, the command is `kill 140`.
...
The `nohup` command starts a process so that it ignores the `SIGHUP` signal sent when the session ends. It also redirects the process's output to a file named `nohup.out` in the directory where the command was executed.

::: terminal
{{USERNAME}}@{{HOSTNAME}}:~$ nohup sleep 1000 &
[1] 141
nohup: ignoring input and appending output to 'nohup.out'
{{USERNAME}}@{{HOSTNAME}}:~$ █
:::

Exit and re-enter the terminal session, as we did with the `disown` command exercise.
